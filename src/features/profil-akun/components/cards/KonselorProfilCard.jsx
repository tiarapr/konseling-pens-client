...
export default function KonselorProfileCard() {
  ...
  // Fetch konselor data
  const fetchKonselorProfile = async () => {
    try {
      const response = await api.get("/konselor-profil/me");
      const profil = response.data.data.konselorProfil;
      setKonselor(profil);
      setEditedData({
        nama_lengkap: profil.nama_lengkap,
        spesialisasi: profil.spesialisasi,
        sipp: profil.sipp
      });
    } catch (error) {
      console.error("Error fetching konselor profile data:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    setRole(user.role_name || "konselor");

    fetchKonselorProfile();
  }, []);
  ...
  const saveProfile = async () => {
    setIsSaving(true);
    try {
      const response = await api.patch(`/konselor-profil/${konselor.id}`, editedData);
      setKonselor(response.data.data.konselorProfil);
      setIsEditing(false);
      alert("Profile updated successfully!");

      await fetchKonselorProfile();
    } catch (error) {
  ...
  const uploadPhotoHandler = async (e) => {
    ...
    try {
      const response = await api.post(`/konselor-profil/${konselor.id}/photo`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      setKonselor((prevkonselor) => ({
        ...prevkonselor,
        photo_url: response.data.data.konselorProfil.photo_url,
      }));
            ...
            {isEditing ? (
              <div className="space-y-4">
                ...
                <Input
                  type="text"
                  name="sipp"
                  value={editedData.sipp}
                  onChange={handleInputChange}
                  className="text-sm text-gray-500 dark:text-gray-400"
                  placeholder="SIPP Number"
                />
              </div>
            ) : (
              <div>
                <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90 text-left">
                  {konselor.nama_lengkap || "Name not available"}
                </h4>
                <p className="text-sm text-gray-500 dark:text-gray-400">{role}</p>

                <div className="text-left mt-4 space-y-4">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    <strong>SIPP:</strong> {konselor.sipp || "N/A"}
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    <strong>Specialization:</strong> {konselor.spesialisasi || "Not specified"}
                  </p>
                </div>
                <div className="flex flex-col gap-1 mt-4 space-y-4 xl:flex-row xl:gap-3 text-left">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    <strong>Joined on:</strong> {new Date(konselor.created_at).toLocaleDateString()}
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    <strong>Last updated:</strong> {new Date(konselor.updated_at).toLocaleDateString()}
                  </p>
                </div>
              </div>
            )}
