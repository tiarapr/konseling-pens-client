...
import { getAverageRating } from "../../../services/DashboardService"; // Pastikan untuk menggunakan path yang benar untuk API

export default function Rating() {
  const [ratingData, setRatingData] = useState(null);  // Menyimpan data rating
  const [isLoading, setIsLoading] = useState(true);    // Menyimpan status loading

  useEffect(() => {
    const fetchRatingData = async () => {
      try {
        const data = await getAverageRating(); // Mengambil data dari API
        setRatingData(data.data); // Menyimpan data yang diterima
        setIsLoading(false);  // Mengubah status loading menjadi false setelah data didapat
      } catch (error) {
        console.error("Error fetching rating data:", error);
        setIsLoading(false);  // Mengubah status loading menjadi false meski ada error
      }
    };
